...
class DeliveryBoyOrderDetails extends StatefulWidget {
  ...
  @override
  _DeliveryBoyOrderDetailsState createState() =>
      _DeliveryBoyOrderDetailsState();
}
...
class _DeliveryBoyOrderDetailsState extends State<DeliveryBoyOrderDetails> {
      ...
      setState(() {
        ...
        isDelivered =
            orderProcessClass.orderDetails[0].deliveredStatus == "Activate"
                ? true
                : false;
        isCanceled = orderProcessClass.orderDetails[0].cancelDateTime != null
            ? true
            : false;
      });
    } catch (e) {
  ...
  getOrderDetails() async {
    ...
    final response = await get(
            "$SERVER_ADDRESS/api/order_item_details?order_id=${widget.id}")
        .catchError((e) {
      setState(() {
        isErrorInLoading = true;
      });
                                      ...
                                      Row(
                                        children: [
                                          InkWell(
                                            onTap: () {
                                              launch(
                                                  "tel://${orderProcessClass.orderDetails[0].contact}");
                                            },
                                            child: Image.asset(
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        "$CURRENCY${double.parse(orderProcessClass.orderDetails[0].deliveryCharges).toStringAsFixed(2)}",
                                        style: TextStyle(
                                            fontFamily: 'Bergen',
                                            fontWeight: FontWeight.w900,
  ...
  void picked() async {
    ...
    final response = await get(url).catchError((e) {
    ...
    try {
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == "1") {
          messageDialogLocal(SUCCESSFUL, ORDER_HAS_BEEN_PICKED_UP_SUCCESSFULLY);
        }
      }
    } catch (e) {
      messageDialog(ERROR, SOMETHING_WENT_WRONG, context);
  ...
  void orderCancel() async {
    ...
    try {
      if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        if (jsonResponse['success'] == "1") {
          messageDialogLocal(SUCCESSFUL, ORDER_HAS_BEEN_CLOSED_SUCCESSFULLY);
        }
      }
    } catch (e) {
      messageDialog(ERROR, SOMETHING_WENT_WRONG, context);
  ...
  deliverOrder() async {
    ...
    try {
      if (response.statusCode == 200) {
        final jsonResponse = jsonDecode(response.body);
        if (jsonResponse['success'] == "1") {
          messageDialogLocal(SUCCESSFUL, ORDER_HAS_BEEN_DELIVERED_SUCCESSFULLY);
        }
      }
    } catch (e) {
      messageDialog(ERROR, SOMETHING_WENT_WRONG, context);
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  ARE_YOU_SURE_TO_PICK_THIS_ORDER,
                  style: TextStyle(
                    fontFamily: 'Bergen',
                    color: BLACK,
                    fontSize: 15,
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  ARE_YOU_SURE_TO_MARK_THIS_ORDER_AS_DELIVERED,
                  style: TextStyle(
                    fontFamily: 'Bergen',
                    color: BLACK,
                    fontSize: 15,
  ...
  messageDialogLocal(String s1, String s2) {
    getOrderStatus();
    return showDialog(
        context: context,
        builder: (context) {
